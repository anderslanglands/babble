...
using test048_Bar = unsigned int;
...
int test048_IntBarPair_second(test048_IntBarPair_t* p, unsigned int** _result) {
    try {
        *_result = reinterpret_cast<unsigned int*>(&bblext::IntBarPair_second(*p));
        return 0;
    } catch (std::exception& e) {
        _bbl_error_message = e.what();
        return 1;
...
int test048_IntBarPair_second_const(test048_IntBarPair_t const* p, unsigned int const** _result) {
    try {
        *_result = reinterpret_cast<unsigned int const*>(&bblext::IntBarPair_second_const(*p));
        return 0;
    } catch (std::exception& e) {
        _bbl_error_message = e.what();
        return 1;
