...
auto extract_field_from_decl_ref_expr(clang::DeclRefExpr const* dre,
                                      bbl::Context* bbl_ctx,
                                      clang::ASTContext* ast_context,
                                      clang::SourceManager& sm,
                                      clang::MangleContext* mangle_context)
...
auto extract_function_from_decl_ref_expr(clang::DeclRefExpr const* dre_fn,
                                         bbl::Context* bbl_ctx,
                                         clang::ASTContext* ast_context,
                                         clang::SourceManager& sm,
                                         clang::MangleContext* mangle_context)
...
auto extract_ctor_from_member_call_expr(clang::CXXMemberCallExpr const* mce,
                                        bbl::Context* bbl_ctx,
                                        clang::ASTContext* ast_context,
                                        clang::SourceManager& sm,
                                        clang::MangleContext* mangle_context)
    ...
    bbl_ctx->insert_constructor_binding(ctor_id, Constructor{
                                                     rename_str,
                                                     comment,
                                                     std::move(parameters),
                                                     is_noexcept,
...
auto extract_replacement_type_from_member_call_expr(
    clang::CXXMemberCallExpr const* mce, bbl::Context* bbl_ctx,
    clang::ASTContext* ast_context, clang::SourceManager& sm,
...
void ExtractMethodBindings::run(
    ...
    if (clang::DeclRefExpr const* dre =
        ...
        extract_method_from_decl_ref_expr(dre, _bbl_ctx, ast_context, sm,
                                          mangle_context.get());
    } else if (clang::DeclRefExpr const* dre =
                   result.Nodes.getNodeAs<clang::DeclRefExpr>(
                       "DeclRefExpr(field)")) {
        extract_field_from_decl_ref_expr(dre, _bbl_ctx, ast_context, sm,
                                         mangle_context.get());
    } else if (clang::DeclRefExpr const* dre_fn =
                   result.Nodes.getNodeAs<clang::DeclRefExpr>(
                       "DeclRefExpr(fn)")) {
        extract_function_from_decl_ref_expr(dre_fn, _bbl_ctx, ast_context, sm,
                                            mangle_context.get());
    } else if (clang::CXXMemberCallExpr const* mce =
                   result.Nodes.getNodeAs<clang::CXXMemberCallExpr>("Ctor")) {
        extract_ctor_from_member_call_expr(mce, _bbl_ctx, ast_context, sm,
                                           mangle_context.get());
    } else if (clang::CXXMemberCallExpr const* mce =
                   result.Nodes.getNodeAs<clang::CXXMemberCallExpr>(
                       "replace_with")) {
        extract_replacement_type_from_member_call_expr(
            mce, _bbl_ctx, ast_context, sm, mangle_context.get());
    }
}

auto extract_class_from_construct_expr(
    clang::CXXConstructExpr const* construct_expr, bbl::Context* bbl_ctx,
    clang::ASTContext* ast_context, clang::SourceManager const& sm,
    clang::MangleContext* mangle_context) -> void {
    ...
    if (type_record_decl->getNameAsString() == "function" &&
        ...
        try {
            StdFunction fun = bbl_ctx->extract_stdfunction_binding(
                dyn_cast<clang::ClassTemplateSpecializationDecl>(
                    type_record_decl),
                class_spelling, rename_str, comment, mangle_context);
...
auto extract_enum_from_construct_expr(
    clang::CXXConstructExpr const* construct_expr, bbl::Context* bbl_ctx,
    clang::ASTContext* ast_context, clang::SourceManager const& sm,
    clang::MangleContext* mangle_context) -> void {
    // Find the class decl that we're binding from the bbl::Class<Foo>()
...
auto extract_class_from_var_decl(clang::VarDecl const* vd,
                                 bbl::Context* bbl_ctx,
                                 clang::ASTContext* ast_context,
    ...
    if (vd_range_str.find("=") != std::string::npos) {
        // this is an assignment declaration, so the spelling captured
        // is already correct
        return;
    }
    ...
    auto const* crd_target = template_args[0].getAsType()->getAsCXXRecordDecl();
    if (crd_target == nullptr) {
        BBL_THROW("could not get CXXRecordDecl from template arg in {}",
                  vd_range_str);
    }

    std::string class_id = get_mangled_name(crd_target, mangle_context);
    Class* cls = bbl_ctx->get_class(class_id);
    if (cls == nullptr) {
    ...
    clang::SourceRange bbl_type_range(vd_range.getBegin(),
                                      init_expr->getSourceRange().getBegin());
    std::string bbl_type_str = get_source_text(bbl_type_range, sm);

    // so now our type string should just be the bit between the first
    // and last <>
...
auto extract_rename_namespace_from_call_expr(
    clang::CallExpr const* ce, bbl::Context* bbl_ctx,
    clang::ASTContext* ast_context, clang::SourceManager const& sm,
    clang::MangleContext* mangle_context) -> void {
    ...
    std::string mod_id;
    if (!find_containing_module(ce, ast_context, mangle_context, mod_id)) {
        BBL_THROW("could not find module containing CallExpr {}",
                  get_source_text(ce->getSourceRange(), sm));
    }

    Module* mod = bbl_ctx->get_module(mod_id);
...
void ExtractClassBindings::run(
    ...
    if (clang::CXXConstructExpr const* construct_expr =
        ...
        extract_class_from_construct_expr(construct_expr, _bbl_ctx, ast_context,
                                          sm, mangle_context.get());
    } else if (clang::CXXConstructExpr const* construct_expr =
                   result.Nodes.getNodeAs<clang::CXXConstructExpr>(
                       "enum_ctor_expr")) {
        extract_enum_from_construct_expr(construct_expr, _bbl_ctx, ast_context,
                                         sm, mangle_context.get());
    } else if (clang::VarDecl const* vd =
                   result.Nodes.getNodeAs<clang::VarDecl>("VarDecl")) {
        extract_class_from_var_decl(vd, _bbl_ctx, ast_context, sm,
                                    mangle_context.get());
    } else if (clang::CallExpr const* ce =
                   result.Nodes.getNodeAs<clang::CallExpr>(
                       "rename_namespace")) {
        extract_rename_namespace_from_call_expr(ce, _bbl_ctx, ast_context, sm,
                                                mangle_context.get());
    }
}
